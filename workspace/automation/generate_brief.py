...
El script busca automáticamente credenciales en el siguiente orden de costo:
  ...
  4. OPENROUTER_API_KEY -> openai/gpt-4o-mini (auto-mapea prefijos)
  5. ANTHROPIC_API_KEY  -> claude-3-5-haiku (evita Opus/Sonnet para ahorrar)
  6. GROQ_API_KEY       -> llama-3.3-70b-versatile
  7. LLM_API_KEY / URL  -> Endpoint genérico compatible con OpenAI (Ollama, etc.)
...
sections:
  - section_type: <hero|features|content-list|cta|stats|testimonials|logos|content>
    ...
    # For repeating/grid sections, add the matching list key:
    # - For features: features
    # - For content-list: items
...
- **NO PLACEHOLDERS**: Do NOT use placeholder texts like 'Título 1', 'Texto 2', 'Descripción', 'Ejemplo', or 'Lorem Ipsum'. Every single title, eyebrow, text, button text, and list item must be fully written out in realistic, premium, and evocative Spanish.
- **EDITORIAL LEVEL**: The copy must feel sophisticated, literary, and high-impact. Use rich vocabulary tailored specifically to the brand identity (e.g. for Bibliotheca, focus on themes of silence, knowledge, history, timelessness, and reading).
- **REALISTIC EXAMPLES**: Make up realistic, elegant articles or features (e.g., instead of 'Título 1' under latest articles, write 'El enigma del scriptorium: el arte del pergamino medieval').
...
def main():
    ...
    parser.add_argument("--provider", help="Force a specific provider (e.g. 'groq', 'openai', 'gemini')")
    ...
    if not providers:
        ...
        print("Configure LLM_API_KEY or any provider-specific API key (e.g. GROQ_API_KEY, GEMINI_API_KEY, etc.)", file=sys.stderr)
        return 1
