...
def compose_background(profile, zone: str) -> Dict[str, Any]:
    ...
    content_pad_top = getattr(profile, 'content_padding_top', '100px')
    content_pad_bottom = getattr(profile, 'content_padding_bottom', '100px')
...
def stagger_delay(index: int, total: int = 6, base_ms: int = 100, style: str = "subtle") -> str:
    if style == "none":
        return "0ms"
    if style == "dramatic":
        return f"{(index % 4) * base_ms + base_ms}ms"
    return f"{(index % 4) * base_ms}ms"
